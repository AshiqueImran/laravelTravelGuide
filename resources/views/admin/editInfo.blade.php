    ...
    <body>
        ...
        <div class="container-fluid editInfoDiv">
            <div class="row marginContentEditDiv">
                <div class="container">
                    <h1 class="text-capitalize headerVisit text-center">edit info</h1>

                @foreach($places as $place)
                    <div class="placesInfo">
                        <h1 class="text-capitalize">place: <span style="color: #27ae60;">{{$place->plcaeName}}</span></h1>
                        <h1 class="text-capitalize">category: <span style="color: #27ae60;">{{$place->category}}</span></h1>
                        <h1 class="text-capitalize">details:</h1>
                        <p>{{$place->details}}</p>
                        <h1 class="text-capitalize">hotel: <span style="color: #27ae60;">{{$place->hotel}}</span></h1>
                        <h1 class="text-capitalize">capacity: <span style="color: #27ae60;">{{$place->capacity}}</span></h1>

                        <div class="text-right">
                            <a class="btn text-capitalize" href="/admin/edit/{{$place->id}}">edit info</a>
                            <a class="btn text-capitalize" onclick="deleteInfo{{$place->id}}()" href="#">delete</a>
                        </div>
                            <script>
                                function deleteInfo{{$place->id}}() {
                                    var txt;
                                    if (confirm("Delete place info of '{{$place->plcaeName}}'??")) 
                                    {
                                      self.location = "/admin/editInfo/del/{{$place->id}}";
